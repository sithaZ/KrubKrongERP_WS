  ...
  <section class="page-shell">
    ...
    <div class="erp-card toolbar-card">
      <div class="toolbar-grid">
        ...
        <div class="form-group">
          <button @click="fetchAttendance" class="erp-btn-secondary" type="button">
            Refresh
          </button>
        </div>
      </div>
        ...
        <table class="erp-table attendance-table">
          ...
          <tbody>
            ...
            <tr v-else v-for="record in filteredAttendance" :key="record._id">
              ...
              <td class="date-time-cell" :title="formatDateTime(record.checkIn)">
                {{ formatDateTime(record.checkIn) }}
              </td>

              <td class="date-time-cell" :title="formatDateTime(record.checkOut)">
              ...
              <td>
                <div class="actions-inline">
                  <button @click="openEditModal(record)" class="erp-btn-soft" type="button">
                    Edit
                  </button>
                </div>
              </td>
        ...
        <form @submit.prevent="saveAttendance" class="modal-form">
          <div class="form-row">
            <div class="form-group">
              ...
              <input v-model="formData.checkIn" type="datetime-local" class="erp-input" />
            </div>

            <div class="form-group">
              <label>Check Out</label>
              <input v-model="formData.checkOut" type="datetime-local" class="erp-input" />
            </div>
          </div>
          ...
          <div class="form-row">
            ...
            <div class="form-group">
              ...
              <input v-model.number="formData.workedHours" type="number" min="0" step="0.01" class="erp-input" />
            </div>
          </div>
...
const saveAttendance = async () => {
  const payload = {
    checkIn: formData.value.checkIn ? new Date(formData.value.checkIn).toISOString() : undefined,
    checkOut: formData.value.checkOut ? new Date(formData.value.checkOut).toISOString() : undefined,
  ...
  try {
    ...
    if (!response.ok) {
      console.error('Save attendance failed:', await response.text())
      return
    }
